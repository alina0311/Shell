...
int breed_process(int in, int out, struct command *cmd)
{
	...
	if ((pid = fork ()) == 0)
    {
		...
		if (in != 0)
        {
        	dup2 (in, 0); //cream o copie pt file descriptorul r
         	close (in);
        }

      	if (out != 1)
        {
          	dup2 (out, 1); //cream o copie pt file descriptorul w
          	close (out);
        }
  	...
  	return pid; //returnez pidul copilului
}
...
int f_pipes(int n, struct command *cmd)
{
	...
	for (i = 0; i < n - 1; ++i)
    {
    	...
    	//cream un pipe: f[0] read, f[1] write
		pipe (fd);
  	...
  	// setam stdin sa fie capatul de r end de la ultimul output din pipe si output catre originalul fisier cu descriptor 1. 
	if (in != 0)
	{
		dup2 (in, 0);
	}
		...
		if(line[0] != 0){
			...
			{	
				...
				do
				{
					...
					//pipecmd[nrp-1].argv[2] = NULL;
					//pipecmd[nrp-1].argv[args] = NULL;
					

					//printf("=%s=%s=%s=%s=%s\n", pipecmd[nrp-1].argv[0], pipecmd[nrp-1].argv[1], pipecmd[nrp-1].argv[2], pipecmd[nrp-1].argv[3], pipecmd[nrp-1].argv[4]);

					//status=execute(cmd);
					/*
					if(status==1)//a a vut eroare
					{
						printf("Ati scris comanda gresit \n");
						break;
					}*/
